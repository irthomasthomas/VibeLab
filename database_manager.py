...
class DatabaseManager:
    def __init__(self, db_path: str = "data/vibelab_research.db"):
        ...
        self.db_path.parent.mkdir(parents=True, exist_ok=True) # Ensure parent directory exists
        # init_database is called by FastAPI app startup or explicitly if needed
        # to avoid issues if schema is read before file exists (e.g. in tests)

    def _connect(self) -> sqlite3.Connection:
        """Establish a database connection."""
        conn = sqlite3.connect(self.db_path, timeout=10) # Added timeout
        conn.row_factory = sqlite3.Row # Set row_factory for all connections from here
        return conn
    ...
    # Prompt operations (for experiment-specific prompts)
    ...
    def register_model(self, name: str, model_type: str = 'base', 
        ...
        # Using INSERT OR REPLACE as per original, though explicit check might be better
        self.execute_commit(""" 
    ...
    # Data migration (simplified - original was complex and might need separate handling)
    def migrate_localStorage_data(self, localStorage_data: Dict):
        ...
        # Consider if this is still needed or if it was a one-off.
        # If needed, it should be carefully tested.
...
# Removed __main__ block, testing should be done with pytest
